# Native Develop Re-entrant Blocker Smoke

This fixture verifies the first real M9 product-control-plane branch: the packaged `workflowprogram-develop.js` must execute through the Claude Code `Workflow` tool and reject an incomplete invocation with a structured blocker.
...
```json
{
  ...
  "runId": "",
  "blockingIssues": [
    "request, targetRoot, runRoot, and runId are required"
  ],
...
- JSONL line 21 reports the structured `BLOCKED_INPUT` result;
...
This is an early blocker-path smoke for M9. The M11 Computer Use harness still needs to cover the complete interactive lifecycle: Skill discovery, clarification, confirmation, agent execution, schema validation, generation, validation, smoke evidence, controlled apply, and final delivery.
