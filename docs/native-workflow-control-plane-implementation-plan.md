...
| Round 66 | FreeSTRIDE 0.1.27 回归发现主 agent 在外部确认后仍进入 Claude Code plan mode，写入 `.claude/plans/*.md` 并调用 `ExitPlanMode`，没有继续产品 Workflow | PreToolUse hook 覆盖 `ExitPlanMode`；foreground guard 拒绝 WPN intent/state 下的 plan-mode 出口和 `.claude/plans/*.md` 写入；入口 Skill 明确外部确认后必须直接调用产品 Workflow |

P8 已完成：真实 Claude Code CLI 中绝对 `scriptPath` 启动成功。早期按产品名称 lookup 失败仅作为历史边界；2026-06-08 已观察到后续 Claude Code 版本可能按 `meta.name` 暴露插件 workflow，因此当前稳定契约是产品 `meta.name=workflowprogram-product-*`，公开入口由 Skill 解析绝对 `scriptPath`。P9-M10C 已完成五个产品 Native JS 的自动化实现；M11 第一版交互式 smoke harness（`build-native-interactive-smoke.py`）已完成 `packet` 与 `evaluate` 子命令，支持人工执行 packet 生成与确定性 JSONL 判读；M12 已完成 Native manifest、发布资格聚合、managed apply no-op 和 drift 阻断。P13（即 M13）已完成：按任务类型选择模型的核心实现，`resolve-task-model-policy.py` 将逻辑 taskType 解析为模型别名并输出 resolution；product workflow JS 通过 `withTaskModel(taskType, options)` 消费映射结果；31 项单元测试覆盖。M14 已完成只读 legacy 下线资格评估，实际删除仍 deferred。M15 已完成 product handoff renderer 收窄，active develop leaf 不再依赖 M7 readiness/authoring 兼容桥。M16 已关闭 rollback/deprecation anchor 与 legacy routing blocker，当前 active blocker 仅剩完整产品交互式 smoke。Computer Use 终端驱动仍处于 deferred 状态，当前仅支持 manual WSL login-shell execution + deterministic JSONL evaluation。不得将宿主侧 renderer 兼容桥解释为第二套 runtime。
